...
from utils.single_fourier_series_value import single_fourier_sequence, single_fourier_value
...
def pa_rotator_genetic_algorithm_fit(data: pd.DataFrame,
                                     fitness_function: Callable[[np.ndarray], float],
                                     name: str,
                                     m_: int = 1,
                                     population_size: int = 500,
    ...
    # Plotting best fit and overall best
    days = data['julian_day'].values
    plt.plot(days, single_fourier_sequence(final_generation[0], m_, days), label='Last Generation Best')
    plt.plot(days, single_fourier_sequence(final_generation[4], m_, days), label='Overall Best')
